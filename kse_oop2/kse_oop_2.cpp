...
public:
	...
	void init_catalog()
	{
		catalog.products = std::move(inv.products);
	}
...
public:
	...
	void add_product_to_inventory(const std::string&typee, const std::string& namee, const double& price, const std::vector<std::string>& details)
	{
		auto createInstance = [](const std::string& type) -> int
		{
			if (type == "Electronics") {
